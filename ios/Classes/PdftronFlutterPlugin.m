        ...
        if(configPairs)
        {
            ...
            for (NSString* key in configPairs.allKeys) {
                ...
                else if([key isEqualToString:PTDisabledElementsKey])
                {
                    ...
                    if (fitMode) {
                        documentController.fitMode = fitMode;
                    ...
                    NSLog(@"%@", layoutMode);
                    if (layoutMode) {
                        NSLog(@"is it happening");
                        NSLog(@"%@", NSStringFromClass([documentController class]));
                        documentController.layoutMode = layoutMode;
                    ...
                    if (overrideLongPressMenuBehavior) {
                    ...
                    if (overrideAnnotationMenuBehavior) {
                    ...
                    if (excludedAnnotationListTypes) {
                    ...
                    if (signSignatureFieldsWithStampsNumber) {
                    ...
                    if (selectAnnotAfterCreationNumber) {
                    ...
                    if (pageIndicatorAlwaysVisibleNumber) {
                    ...
                    if (hideDefaultAnnotationToolbars) {
                    ...
                    if (hideAnnotationToolbarSwitcherNumber) {
                    ...
                    if (thumbnailViewEditingEnabledNumber) {
                    ...
                    if (annotationsListEditingEnabled) {
                    ...
                    if (userBookmarksListEditingEnabled) {
                    ...
                    if (showNavigationListAsSidePanelOnLargeDevices) {
                ...
                else if ([key isEqualToString:PTAnnotationManagerEnabedKey])
                {
                    ...
                    if (annotationManagerEnabledNumber) {
...
+ (id)getConfigValue:(NSDictionary*)configDict configKey:(NSString*)configKey class:(Class)class error:(NSError**)error
{
    ...
    NSLog(@"config: %@, %@", configKey, configResult);
...
- (void)documentControllerDidOpenDocument:(PTDocumentController *)documentController
{
    ...
    if (((PTFlutterDocumentController*)documentController).isDocLoaded == NO) {
        return;
    }
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    } else if ([call.method isEqualToString:PTRemoveBookmarkKey]) {
        NSString *title = [PdftronFlutterPlugin PT_idAsNSString:call.arguments[PTBookmarkTitleArgumentKey]];
        [self removeBookmark:title resultToken:result];
    } else if ([call.method isEqualToString:PTSaveDocumentKey]) {
        NSString *path = [PdftronFlutterPlugin PT_idAsNSString:call.arguments[PTPathArgumentKey]];
        [self saveDocument:path resultToken:result];
    ...
    } else if ([call.method isEqualToString:PTOpenThumbnailBrowserKey]) {
        [self openThumbnailBrowser:result];
    } else if ([call.method isEqualToString:PTSetControlsHiddenKey]) {
        bool *hidden = [PdftronFlutterPlugin PT_idAsBool:call.arguments[PTControlsHiddenArgumentKey]];
        [self setControlsHidden:result hidden:hidden];
    } else if ([call.method isEqualToString:PTSetHorizontalKey]) {
...
- (void)removeBookmark:(NSString *)title resultToken:(FlutterResult)flutterResult
{
    PTDocumentController *documentController = [self getDocumentController];
    if(documentController.document == Nil)
    {
        // something is wrong, no document.
        NSLog(@"Error: The document view controller has no document.");
        flutterResult([FlutterError errorWithCode:@"remove_bookmark" message:@"Failed to remove bookmark" details:@"Error: The document view controller has no document."]);
        return;
    }
    ...
    [documentController.pdfViewCtrl DocLock:YES withBlock:^(PTPDFDoc * _Nullable doc) {
        if([doc HasDownloader])
        {
            // too soon
            NSLog(@"Error: The document is still being downloaded.");
            flutterResult([FlutterError errorWithCode:@"remove_bookmark" message:@"Failed to add bookmark" details:@"Error: The document is still being downloaded."]);
            return;
        }
        ...
        for (PTUserBookmark *item in bookmarks) {
    ...
    if(error)
    {
        NSLog(@"Error: There was an error while trying to remove bookmark. %@", error.localizedDescription);
        flutterResult([FlutterError errorWithCode:@"remove_bookmark" message:@"Failed to add bookmark" details:@"Error: There was an error while trying to remove bookmark."]);
    } else {
        flutterResult(nil);
    }
...
- (void)saveDocument:(NSString *) path resultToken:(FlutterResult)flutterResult
{
    PTFlutterDocumentController *documentController = (PTFlutterDocumentController *)[self getDocumentController];
...
- (void)setHorizontal:(FlutterResult)flutterResult
{
    PTDocumentController *documentController = [self getDocumentController];
...
- (void)setVertical:(FlutterResult)flutterResult
{
    PTDocumentController *documentController = [self getDocumentController];
...
- (void)extractTextForPage:(FlutterResult)flutterResult page:(NSNumber *)pageNumber
{
    NSError *error;
    ...
    [documentController.pdfViewCtrl DocLockReadWithBlock:^(PTPDFDoc * _Nullable doc) {
        ...
        for (; [line IsValid]; line=[line GetNextLine])	{
            for (word=[line GetFirstWord]; [word IsValid]; word=[word GetNextWord]) {
                NSString *newString = [word GetString];
                str = [str stringByAppendingString:@" "];
                str = [str stringByAppendingString:newString];
            }
        }
        flutterResult(str);
        return;
    ...
    if (error) {
        NSLog(@"Error: Failed to extract text from doc. %@", error.localizedDescription);
        flutterResult([FlutterError errorWithCode:@"export_annotations" message:@"Failed to export annotations" details:@"Failed to extract fdf from doc."]);
        return;
    }
...
- (void)setControlsHidden:(FlutterResult)flutterResult hidden:(BOOL)hidden
{
    PTDocumentController *documentController = [self getDocumentController];
    // if (!documentController.bookmarkListHidden) {
    //     PTNavigationListsViewController *navigationListsViewController = documentController.navigationListsViewController;
    ...
    //     }
    // }
    [documentController setControlsHidden:hidden animated:true];
